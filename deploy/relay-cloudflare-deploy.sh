...
#   1. `wrangler login` once, before running this (a browser OAuth — a script can't do it).
...
#
# Everything else — create the D1 database, wire its id into wrangler.toml, apply the schema,
# build the static site, and deploy — is done for you. Safe to re-run: every step is idempotent.
#
# Usage:  npm i -g wrangler && wrangler login    # once
...
wrangler d1 execute "$DB_NAME" --remote --file=./migrations/0001_init.sql --yes
