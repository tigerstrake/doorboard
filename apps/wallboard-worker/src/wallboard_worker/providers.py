...
def build_bird_provider(settings: Settings, *, force_mock: bool = False) -> BirdProvider:
    if force_mock or settings.bird_provider == "mock" or settings.birdnet_url == "mock":
        logger.info("bird_provider_selected", extra={"provider": "mock"})
        return MockBirdProvider()

    if settings.bird_provider == "avian_visitors":
        logger.info(
            "bird_provider_selected",
            extra={"provider": "avian_visitors", "url": settings.avian_visitors_url},
        )
        return AvianVisitorsProvider(
            AvianVisitorsConfig(
    ...
    logger.info(
        "bird_provider_selected",
        extra={"provider": "birdnet_go", "url": settings.birdnet_url},
    )
